...
/*
ou are given five integers cost1, cost2, costBoth, need1, and need2.
...
public:
    long long minimumCost(int cost1, int cost2, int costBoth, int need1, int need2)
    {
        ...
        long long take1 = (long long)cost1 * need1;
        take1 = take1 + (long long)cost2 * need2;

        long long take2 = (long long)max(need1, need2) * costBoth;

        long long take3 = (long long)min(need1, need2) * costBoth;

        if (need1 > need2)
        {
            take3 += (long long)(need1 - need2) * cost1;
        }
        else
        {
            take3 += (long long)(need2 - need1) * cost2;
        }

        return min({take1, take2, take3});
    }
};
